...
use Xatoma\SteamCommunity\TradeOffers\TradeOffer\Item;
    ...
    private function getItemDetailsFromInventory($steamId, $items)
    {
        ...
        foreach ($items as &$item) {
            ...
            if (array_key_exists($item->getAssetId(), $inventoryCache[$identifier])) {
                ...
                if (!empty($invItem['name'])) {
                    $item->setName($invItem['name']);
                }
                if (!empty($invItem['market_name'])) {
                    $item->setMarketName($invItem['market_name']);
                }
                if (!empty($invItem['type'])) {
                    $item->setType($invItem['type']);
                }
                if (!empty($invItem['icon_url'])) {
                    $item->setIconUrl($invItem['icon_url']);
                }
                if (!empty($invItem['icon_url_large'])) {
                    $item->setIconUrlLarge($invItem['icon_url_large']);
                }
                if (!empty($invItem['name_color'])) {
                    $item->setColor($invItem['name_color']);
                }
            }
        }
